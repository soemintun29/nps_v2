...
export const ExecutiveDashboard: React.FC<DashboardProps> = ({ completedCalls }) => {
  const stats = useMemo(() => {
    ...
    const validCalls = completedCalls.filter(c => c.status !== 'refused');
    ...
    const totalAll = completedCalls.length;
    ...
    validCalls.forEach(c => {
      ...
      branchMap[branch].score += (c.nps_score ?? 0);
    ...
    return { nps, csat, total: totalAll, branchData, techData, promoters, detractors, passives: totalValid - promoters - detractors };
  }, [completedCalls]);
