...
export const AdminPanel: React.FC = () => {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (!window.confirm("WARNING: Creating a user here will temporarily log you out of your Admin session. You will need to log back in. Proceed?")) {
      return;
    }

    setLoading(true);
    const email = formData.username.trim().includes('@') ? formData.username.trim() : `${formData.username.trim()}@midea-internal.com`;
    ...
    try {
      ...
      if (authData.user) {
        // Use upsert to ensure the profile is created/updated immediately
        ...
        alert(`User ${formData.username} created successfully! You will now be logged out. Please log back in with your Admin credentials.`);
